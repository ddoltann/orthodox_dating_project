...
def profile_list(request):
    ...
    if form.is_valid():
        ...
        if cd.get('min_age'):
            profiles = profiles.filter(date_of_birth__year__lte=timezone.now().year - cd['min_age'])
        if cd.get('max_age'):
            profiles = profiles.filter(date_of_birth__year__gte=timezone.now().year - cd['max_age'])
    return render(request, 'profiles/profile_list.html', {'profiles': profiles, 'form': form})

@login_required
def profile_detail(request, pk):
    profile_user = get_object_or_404(User, pk=pk)
    has_sent_like = Like.objects.filter(user_from=request.user, user_to=profile_user).exists()
    has_received_like = Like.objects.filter(user_from=profile_user, user_to=request.user).exists()
    mutual_like = has_sent_like and has_received_like
...
def add_like(request, pk):
    ...
    if created:
        ...
        messages.info(request, f'Вы уже выражали симпатию этому пользователю.')
...
def inbox(request):
    ...
    interlocutors_ids = set(list(sent_to) + list(received_from))
    interlocutors = User.objects.filter(id__in=interlocutors_ids).distinct()
    return render(request, 'profiles/inbox.html', {'interlocutors': interlocutors})

@login_required
def conversation_detail(request, pk):
    interlocutor = get_object_or_404(User, pk=pk)
    has_mutual_like = Like.objects.filter(user_from=request.user, user_to=interlocutor).exists() and Like.objects.filter(user_from=interlocutor, user_to=request.user).exists()
    if not has_mutual_like:
